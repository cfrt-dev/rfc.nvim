...
class RFCParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if aid:
            if tag in BLOCK:
                # Block element: flush preceding content, then anchor starts here.
                self._enl()
                self.anch[aid] = len(self.out)
            else:
                # Inline element (e.g. <a id="ref-FOO"> inside <pre>): anchor
                # sits on the current in-progress line — do NOT flush, otherwise
                # the "[" that precedes the id tag ends up on its own line.
                self.anch[aid] = len(self.out)
        ...
        if tag == "a":
            ...
            if href:
                self.stk.append(
                    {"href": href, "line": len(self.out), "col": len(self.cur)}
                )
